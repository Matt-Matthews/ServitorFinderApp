...
async function getRequests() {
  setIsLoading(true)
    ...
    try {
      let ourQuery = query(collectionRef,where('userId','==',userId));
      const servitorRef = collection(firestore, 'servitors')
      onSnapshot(ourQuery, (snapshot) => {
        let tempData = [];
        snapshot.docs.forEach(async (doc) => {
          let servitorQuery = query(servitorRef, where('userId', '==', doc.data().servitorId));
        ...
        <View style={styles.container}>
          ...
          <ScrollView>
            ...
            requests.length!==0?requests.map((req,key)=>{
                      return<ServitorCard req={req} index={key} />
