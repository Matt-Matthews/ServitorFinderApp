...
export default function ServitorCard({isOnline, req,index}) {
   
    const deleteRequest = async ()=>{
      await deleteDoc(doc(firestore, "Request Form", req.docId));
    }
    const cancelRequest = async ()=>{
      await updateDoc(doc(firestore, "Request Form", req.docId),{reqStatus: 'Cancelled'});
    }
    ...
    return (
      ...
      <View style={styles.textContainter}>
        ...
        <Text style={styles.text}>{req.serviceType}</Text>
        <Ratings ratings={3.5} />
      
      </View>

      <Pressable onPress={req.reqStatus==="Pending"?cancelRequest:deleteRequest} style={styles.btnPos}>
        <LinearGradient  
                style={{
                        ...styles.btn 
                    }}
                ...
                <Text style={{...styles.btnText}}>{req.reqStatus==="Pending"?'Cancel':'Delete'}</Text>
            </LinearGradient>
